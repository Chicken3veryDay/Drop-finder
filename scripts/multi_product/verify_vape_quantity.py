...
def _validate_published_vape(product: dict[str, Any], index: int) -> None:
    ...
    source_text = " ".join(
        ...
        for field in ("name", "source_title", "variant", "source_weight_label")
    )
...
def _validate_rejections(rejections: list[dict[str, Any]]) -> Counter[str]:
    ...
    for index, row in enumerate(rejections):
        ...
        if primary_type in SUPPORTED_VAPE_TYPES and mass_only:
            if reason != "unsupported_vape_mass_quantity":
                raise VapeQuantityVerificationError(
                    f"mass-only vape rejection {index} uses unstable reason: {reason or 'missing'}"
                )
        if reason.startswith(("unsupported_vape_", "missing_vape_", "inconsistent_vape_")):
            if reason not in STABLE_QUANTITY_REASONS:
        ...
        if reason == "unsupported_vape_mass_quantity" and not MASS_LABEL.search(source_text):
            raise VapeQuantityVerificationError(
                f"mass-quantity rejection {index} does not retain explicit source mass evidence"
            )
    return counts


def verify_vape_quantity_artifacts(
    catalog_path: Path,
    status_path: Path,
    *,
    source_commit: str = "",
    artifact_label: str = "generated",
) -> dict[str, Any]:
    ...
    if isinstance(declared_rejection_counts, dict):
        for reason, count in declared_rejection_counts.items():
            if reason in STABLE_QUANTITY_REASONS and rejection_counts.get(reason, 0) != int(count):
                raise VapeQuantityVerificationError(
                    f"catalog rejection count disagrees with status for {reason}"
                )
    ...
    generation = str(
        catalog.get("generation_id")
        or catalog.get("generated_at")
        or status.get("generation_id")
