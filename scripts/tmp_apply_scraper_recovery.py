...
route_path = Path("scripts/route_repair.py")
route_text = route_path.read_text(encoding="utf-8")
if "# SCRAPER_RECOVERY_CLOSURE_V1" not in route_text:
    route_path.write_text(route_text.rstrip() + ROUTE_EXTENSION + "\n", encoding="utf-8")

test_path = Path("tests/catalog_v4/test_route_repair.py")
test_text = test_path.read_text(encoding="utf-8")
if "def test_recovery_routes_price_and_invalid_sibling_pruning" not in test_text:
    anchor = '\n\nif __name__ == "__main__":\n'
    if anchor not in test_text:
        raise SystemExit("test insertion anchor missing")
    test_path.write_text(test_text.replace(anchor, "\n" + TEST_INSERTION + anchor, 1), encoding="utf-8")
