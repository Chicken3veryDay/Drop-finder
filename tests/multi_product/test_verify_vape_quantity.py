...
from scripts.multi_product.verify_vape_quantity import VapeQuantityVerificationError, verify_vape_quantity_artifacts
...
class VapeQuantityArtifactVerifierTests(unittest.TestCase):
    def write_artifacts(self, root: Path, products, rejections):
        ...
        status = {
            ...
            "summary": {"active_sources": 3, "degraded_sources": 0},
    ...
    def test_valid_generated_artifacts_produce_authenticated_receipt(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            catalog_path, status_path = self.write_artifacts(
                ...
                [{"primary_type": "cannabis_vape", "name": "Mass-only disposable 2g", "reason": "unsupported_vape_mass_quantity"}],
            )
            receipt = verify_vape_quantity_artifacts(catalog_path, status_path, source_commit="abc123", artifact_label="generated-main")
    ...
    def test_mass_only_vape_cannot_reach_publication(self):
        ...
        invalid.update({"name": "Mass-only disposable 2g", "grams": 2, "volume_ml": None, "quantity_value": 2, "quantity_unit": "g", "comparison_metric": "price_per_gram", "comparison_price": 10, "price_per_ml": None})
        with tempfile.TemporaryDirectory() as directory:
    ...
    def test_mass_only_rejection_requires_stable_reason_and_source_evidence(self):
        with tempfile.TemporaryDirectory() as directory:
            catalog_path, status_path = self.write_artifacts(Path(directory), [], [{"primary_type": "cannabis_vape", "name": "Mass-only disposable 2g", "reason": "missing_vape_volume"}])
            with self.assertRaisesRegex(VapeQuantityVerificationError, "unstable reason"):
                verify_vape_quantity_artifacts(catalog_path, status_path)
        with tempfile.TemporaryDirectory() as directory:
            catalog_path, status_path = self.write_artifacts(Path(directory), [], [{"primary_type": "cannabis_vape", "name": "Vape with unknown quantity", "reason": "unsupported_vape_mass_quantity"}])
