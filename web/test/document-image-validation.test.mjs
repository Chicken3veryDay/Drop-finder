...
test('image-classified HTML responses fail before publishing a ready preview', async () => {
  await trackObjectUrlRevocations(async revoked => {
    ...
    await viewer.close();
    assert.equal(revoked.length, 1, 'failed object URL is not retained for close to revoke again');
  });
});

test('corrupt bytes labeled as an image fail through the same bounded decode gate', async () => {
  await trackObjectUrlRevocations(async revoked => {
    let decodeCalls = 0;
    const viewer = new DocumentViewerCapability({
