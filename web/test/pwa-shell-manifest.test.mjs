...
test('static Vite asset traversal excludes dynamic document entries', () => {
  const manifest = {
    ...
    'admin.html': {
      file: 'assets/admin-SECONDARY.js',
      isEntry: true,
      css: ['assets/admin-SECONDARY.css'],
    },
  };
...
test('current publication shell excludes dynamic Vite outputs within the install budget', async () => {
  const root = resolve(new URL('../../cloud_pages/', import.meta.url).pathname);
  const viteManifest = JSON.parse(await readFile(resolve(root, 'assets/vite-manifest.json'), 'utf8'));
  const shell = await buildShellManifest(root);
  const shellAssets = new Set(shell.assets.map(path => path.replace(/^\.\//, '')));
  const dynamicAssets = new Set();
  for (const record of Object.values(viteManifest)) {
    if (record?.isDynamicEntry !== true) continue;
    if (record.file) dynamicAssets.add(record.file);
    for (const asset of record.assets ?? []) dynamicAssets.add(asset);
    for (const css of record.css ?? []) dynamicAssets.add(css);
  }

  for (const asset of dynamicAssets) {
    assert.equal(shellAssets.has(asset), false, `dynamic Vite output entered the install shell: ${asset}`);
  }
  assert.equal(shell.assets.filter(path => /marketplace-query-worker-/.test(path)).length, 1);
  const installBytes = shell.records.reduce((sum, record) => sum + record.bytes, 0);
  assert.ok(installBytes <= 400 * 1024, `application shell exceeded 400 KiB: ${installBytes}`);
});
