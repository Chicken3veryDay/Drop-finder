...
export const verifyPublication = async (root = publicationRootFrom(projectRoot)) => {
  ...
  for (const tag of parseStartTags(index)) {
    ...
    localReferences.push({ tag: tag.name, rel, type: (tag.attributes.get("type") || "").toLowerCase(), path: safePath });
  }
  ...
  for (const [key, record] of manifestRecords) {
    ...
    for (const field of [record.file, ...css, ...assets]) {
      ...
      const manifestReference = field.startsWith("./") || field.startsWith("../") || field.startsWith("/") ? field : `./${field}`;
      const localPath = localPublicationPath(manifestReference, `Vite manifest record ${key}`);
      if (!localPath) throw new Error(`Vite manifest record ${key} must reference a local publication asset: ${field}`);
  ...
  for (const asset of appShell.assets) {
    ...
    if (!localPath) throw new Error(`Application shell manifest must reference a local asset: ${asset}`);
