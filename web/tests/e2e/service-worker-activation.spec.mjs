...
test('fresh live worker activates the current catalog generation', async ({ page }) => {
  const pageErrors = [];
  page.on('pageerror', error => pageErrors.push(String(error)));
  await page.addInitScript(() => {
    window.__dropfinderMessages = [];
    navigator.serviceWorker?.addEventListener('message', event => {
      window.__dropfinderMessages.push(event.data);
    });
  });

  const nonce = `${Date.now()}-${Math.random().toString(16).slice(2)}`;
  await page.goto(`./?activation_v4=${nonce}`, { waitUntil: 'domcontentloaded' });
  ...
  const release = await page.evaluate(async token => {
    const [workerResponse, manifestResponse, runtimeResponse] = await Promise.all([
      fetch(`./sw.js?activation_v4=${token}`, { cache: 'no-store' }),
      fetch(`./data/catalog-v4/manifest.json?activation_v4=${token}`, { cache: 'no-store' }),
      fetch(`./data/runtime.json?activation_v4=${token}`, { cache: 'no-store' }),
    ]);
  ...
  const results = page.getByRole('list', { name: /marketplace results/i }).first();
  ...
  const messages = await page.evaluate(() => window.__dropfinderMessages ?? []);
